...
parser.add_argument('--output',           default='./output/LH',        help='Output folder.')
parser.add_argument('--dataset',          default='./data/HED-BSDS/LH', help='HED-BSDS dataset folder.')
# 5. Others.
...
def wave_trans(images):
    ...
    # LH is an hw numpy array

    # convert LH into a torch tensor of NCHW

    height = LH.shape[0]
    width = LH.shape[1]
    LH = torch.from_numpy(LH)
    LH = torch.reshape(LH,(1, 1, height, width))
...
def train(train_loader, net, opt, lr_schd, epoch, save_dir):
    ...
    for batch_index, (images, edges) in enumerate(tqdm(train_loader)):
        ...
        # take DWT of image here and set edges to LH
