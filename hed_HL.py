...
parser.add_argument('--output',           default='./output/HL',        help='Output folder.')
parser.add_argument('--dataset',          default='./data/HED-BSDS/HL', help='HED-BSDS dataset folder.')
# 5. Others.
...
def wave_trans(images):
    ...
    # HL is an hw numpy array

    # convert HL into a torch tensor of NCHW

    height = HL.shape[0]
    width = HL.shape[1]
    HL = torch.from_numpy(HL)
    HL = torch.reshape(HL,(1, 1, height, width))
...
def train(train_loader, net, opt, lr_schd, epoch, save_dir):
    ...
    for batch_index, (images, edges) in enumerate(tqdm(train_loader)):
        ...
        # take DWT of image here and set edges to HL
